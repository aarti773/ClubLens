    ...
    <header className="border-b border-white/10 bg-slate-950/90 text-white">
        ...
        <div className="hidden items-center gap-4 text-sm text-slate-300 md:flex">
          ...
          {isLoggedIn ? (
            <>
              ...
              <Link to="/upload" className="rounded-lg bg-white px-4 py-2 font-medium text-slate-950">
                Upload
              </Link>
            </>
          ) : (
            <Link to="/login" className="rounded-lg bg-white px-4 py-2 font-medium text-slate-950">
              Sign in
            </Link>
          )}
        </div>
      ...
      {isMenuOpen && (
        <div className="space-y-4 border-t border-white/10 px-6 py-5 text-sm text-slate-300 md:hidden">
          ...
          {isLoggedIn ? (
            <>
              <button onClick={logout} className="block hover:text-white">Logout</button>
              <Link to="/upload" className="block rounded-lg bg-white px-4 py-2 font-medium text-slate-950">
                Upload
              </Link>
            </>
          ) : (
            <Link to="/login" className="block rounded-lg bg-white px-4 py-2 font-medium text-slate-950">
              Sign in
            </Link>
          )}
        </div>
